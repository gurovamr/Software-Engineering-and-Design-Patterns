...
class TestSpeedChart:
    def test_speed_chart_renders(self):
        df = pd.DataFrame({"Distance": [0, 10], "Speed": [100, 150], "LapNumber": [1, 1]})
...
class TestLapSummaryChart:
    def test_lap_summary_chart_renders(self):
        df = pd.DataFrame({"LapNumber": [1, 2, 3], "LapTimeSeconds": [90.0, 89.5, 90.2]})
...
class TestTrackMapChart:
    def test_track_map_chart_renders(self):
        df = pd.DataFrame({"X": [0.0, 1.0], "Y": [0.0, 1.0], "Speed": [100, 120], "LapNumber": [1, 1]})
...
class TestTeamPaceChart:
    def test_team_pace_chart_renders(self):
        df = pd.DataFrame({"Team": ["Red Bull", "Red Bull"], "LapTimeSeconds": [89.5, 90.0]})
...
class TestLapTimesDistributionChart:
    def test_lap_times_distribution_renders(self):
        df = pd.DataFrame({"Driver": ["VER", "VER", "HAM"], "LapTimeSeconds": [89.5, 90.0, 90.2]})
        fig = LapTimesDistributionChart(df, topn=2).render()
        assert isinstance(fig, go.Figure)
